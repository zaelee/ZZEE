...
test("Mureka Style용 마스터와 트랙 합본은 사용자 입력이 길어도 1000자를 넘지 않는다", () => {
  assert.equal(engine.murekaStyleLimit, 1000);

  for (const preset of Object.values(presets)) {
    const fields = fieldsFor(preset);
    for (const key of ["genres", "bpm", "key", "soundBalance", "musicExclusions", "crossfade"]) {
      fields[key] = `${fields[key]} ${"extended-input ".repeat(500)}`;
    }
    const result = engine.generate(preset, fields);

    assert.ok(result.masterPrompt.length <= 720, `${preset.meta.id} master: ${result.masterPrompt.length}`);
    result.trackPrompts.forEach(({ prompt, index }) => {
      assert.ok(prompt.length <= engine.murekaStyleLimit, `${preset.meta.id} track ${index}: ${prompt.length}`);
      assert.ok(prompt.startsWith(result.masterPrompt));
      assert.match(prompt, /FINAL ENFORCEMENT.*100 percent instrumental/i);
    });
  }
});
